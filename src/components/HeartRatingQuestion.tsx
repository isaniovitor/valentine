...
const HEART_LABELS: Record<number, string> = {
  ...
  3: "Dá pra melhorar",
  4: "Gostei muito",
  5: "Não consigo parar de pensar em você.",
};
...
export const HeartRatingQuestion: React.FC<HeartRatingQuestionProps> = ({
  ...
  const selectedHearts = selectedAnswer
    ? (question.options.find((opt) => opt.letterSegment === selectedAnswer)
        ?.hearts ?? null)
    : null;
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      <div className="flex items-center justify-center gap-2 sm:gap-3 md:gap-4">
        {Array.from({ length: question.maxHearts }, (_, i) => i + 1).map(
          (hearts) => {
            const option = getOptionForHearts(hearts);
            const isFilled = activeHearts !== null && hearts <= activeHearts;
            const isAnimating =
              option !== undefined && animatingSegment === option.letterSegment;
